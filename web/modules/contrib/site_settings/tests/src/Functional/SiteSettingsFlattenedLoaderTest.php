 ...
 * @group SiteSettings
 */
class SiteSettingsFlattenedLoaderTest extends BrowserTestBase {
   ...
   * statements to the browser if the arrays match our expectations. In this
   * way we can catch if any changes to the code are modifying the output of
   * the array as that would result in a breaking change for users of this
   * module.
   */
  public function testSiteSettingsLoaderFormat() {
    // Open the site settings sample data controller.
    $this->drupalGet('site_settings_sample_data/test_flattened_site_settings_loader');
    $session = $this->assertSession();

    // Make sure the groups match.
    $session->pageTextContains('Groups match expectations');
   ...
   */
  public function testTemplateDoNotLoadSiteSettingsWhenDisabled() {
    ...
    $rendered = $renderer->renderInIsolation($renderable);
  ...
  public function testSiteSettingsLoadByEntityBundleClass() {
    ...
    $results = $loader->loadByEntityBundleClass(TestMultipleEntriesSiteSetting::class);
    $this->assertSame([
      'other' => [
