...
class SiteSettingReplicateForm extends FormBase {
   ...
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state, $setting = FALSE): array {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($this->entityTypeManager->getStorage('site_setting_entity_type')->loadMultiple()) {
  ...
  public function machineNameExists($value, array $form, FormStateInterface $form_state): bool {
    ...
    $values = $form_state->getValues();
    foreach ($values['new_settings'] as $setting) {
   ...
   * Handle adding new.
   */
  private function addNewFields(array &$form, FormStateInterface $form_state): void {
   ...
   * Handle submit.
   */
  private function finalSubmit(array &$form, FormStateInterface $form_state): void {

    // Path to the batch processing.
    $path = $this->extensionList->getPath('site_settings');
